...
public class CommandProcessorImpl implements CommandProcessor {
    ...
    public CommandProcessorImpl(Logger logger, ApplicationContext ctx, List<ExceptionHandler> exHandlers) {
        this.logger = logger;
        this.ctx = ctx;
    ...
    @Override
    @Cacheable(cacheNames = "commands", key = "#command.inputHash()")
    public <T extends AbstractCommand> T process(T command) {
        ...
        try {
            ...
            List<CommandHandler> handlersForThisCommand = Stream.of(usecase.handlers())
                    .map(beanClass -> ctx.getBean(beanClass))
